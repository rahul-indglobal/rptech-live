...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const MOBILELOGIN_ALLOWED_COUNTRY = 'mobilelogin/moduleoption/sallowspecific';
    const MOBILELOGIN_SPECIFIC_COUNTRY_LIST = 'mobilelogin/moduleoption/specificcountry';
    ...
    public function isEnable()
    {
        if ($this->isMobileEnable()) {
            if ($this->isAllowedToSpecificCountry()) {
                $selectedCountries = $this->getSpecificCountryList();
                $ipdat = @json_decode(file_get_contents("http://www.geoplugin.net/json.gp?ip=" . $this->remoteAddress->getRemoteAddress()));
                if (!in_array($ipdat->geoplugin_countryCode, $selectedCountries)) {
                    return false;
                }
                return true;
            } else {
    ...
    public function isAllowedToSpecificCountry()
    {
        return $this->scopeConfig->getValue(
            self::MOBILELOGIN_ALLOWED_COUNTRY,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
    public function getSpecificCountryList()
    {
        $list = $this->scopeConfig->getValue(self::MOBILELOGIN_SPECIFIC_COUNTRY_LIST,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    ...
    public function sendOTPCode($mobile, $checkout = 0)
    {
        try {
