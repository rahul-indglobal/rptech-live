...
class Awb extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('delhivery_lastmile_awb', 'awb_id');
    }

    /**
     * Retrieves Manage AWB AWB from DB by passed id.
     *
     * @param string $id
     * @return string|bool
     */
    public function getAwbAwbById($id)
    {
        $adapter = $this->getConnection();
        $select = $adapter->select()
            ->from($this->getMainTable(), 'awb')
            ->where('awb_id = :awb_id');
        $binds = ['awb_id' => (int)$id];
     ...
     *
     * @param \Magento\Framework\Model\AbstractModel|\Delhivery\Lastmile\Model\Awb $object
     * @return $this
     */
