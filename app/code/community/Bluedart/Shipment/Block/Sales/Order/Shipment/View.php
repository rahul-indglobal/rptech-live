 ...
 */
class Bluedart_Shipment_Block_Sales_Order_Shipment_View extends Mage_Adminhtml_Block_Sales_Order_Shipment_View
{
    ...
    public function __construct()
    {
        ...
        if ($newid = $this->getShipment()->getId()) {
			$filenumber = "";
			$shipment = Mage::getModel('sales/order_shipment')->load($newid);
			foreach($shipment->getAllTracks() as $tracknum)
            {
                if($tracknum->getCarrierCode() == 'bluedart')
                {
				    $filenumber = $tracknum->getNumber();
					break;
				}
            }
            if($filenumber)
            {
				$this->_addButton('bluedart', array(
					'label'     => Mage::helper('sales')->__('Generate Blue Dart Label'),
					'class'     => 'save',
					'onclick'   => 'setLocation(\''.Mage::helper('adminhtml')->getUrl('admin_shipment/adminhtml_shipmentbackend/downloadlabel',array('filenumber'=>$filenumber)).'\')'
					)
				);
			}
        }
