...
class Edit extends Action
{
    ...
    public function execute()
    {
        ...
        if ($entityId) {
            $model->load($entityId);
            if (!$model->getId()) {
                $this->messageManager->addErrorMessage(__('This Brand is no longer exists.'));
        ...
        $resultPage->setActiveMenu('Rptech_Brand::brand')
            ->addBreadcrumb(__('Brands'), __('Edit'))
            ->addBreadcrumb(__('Manage Brands'), __('Manage Brands'));
        $resultPage->getConfig()->getTitle()->prepend(__('Brands & Services'));
