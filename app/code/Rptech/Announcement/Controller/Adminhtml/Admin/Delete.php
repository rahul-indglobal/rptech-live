<?php
/**
 * @author Rptech
 * @package Rptech_Announcement
 */
namespace Rptech\Announcement\Controller\Adminhtml\Admin;

use Rptech\Announcement\Model\AnnouncementFactory;
use Exception;
use Magento\Backend\App\Action;
use Magento\Backend\Model\View\Result\Redirect;

/**
 * Class Delete
 * @package Rptech\Announcement\Controller\Adminhtml\Admin
 */
class Delete extends Action
{
    const ADMIN_RESOURCE = 'Rptech_Announcement::delete';

    /**
     * @var AnnouncementFactory
     */
    private $announcementFactory;

    /**
     * Delete constructor.
     * @param Action\Context $context
     * @param AnnouncementFactory $announcementFactory
     */
    public function __construct(
        Action\Context $context,
        AnnouncementFactory $announcementFactory
    ) {
        $this->announcementFactory = $announcementFactory;
        parent::__construct($context);
    }

    public function execute()
    {
        $entityId = $this->getRequest()->getParam('entity_id');
        /** @var Redirect $resultRedirect */
        $resultRedirect = $this->resultRedirectFactory->create();
        if ($entityId) {
            try {
                $model = $this->announcementFactory->create();
                $model->load($entityId);
                $model->delete();
                $this->messageManager->addSuccessMessage(__('The Announcement record has been deleted.'));
                return $resultRedirect->setPath('*/*/');
            } catch (Exception $e) {
                $this->messageManager->addErrorMessage($e->getMessage());
                return $resultRedirect->setPath('*/*/edit', ['entity_id' => $entityId]);
            }
        }
        $this->messageManager->addErrorMessage(__('We can\'t find a Announcement to delete.'));
        return $resultRedirect->setPath('*/*');
    }
}
