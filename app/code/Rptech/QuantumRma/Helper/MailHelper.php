...
namespace Rptech\QuantumRma\Helper;
...
class MailHelper extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function mailSend($emailTemplateVariables, $receiverInfo = [], $type = "otp")
    {
        try {
            if ($type == "otp") {
                $this->temp_id = $this->getConfigValue("rptech/quantumrma/otp_email_template");
            } else {
                $this->temp_id = $this->getConfigValue("rptech/quantumrma/data_email_template");
            }
    ...
    {
        $indent = $this->getConfigValue("rptech/quantumrma/sender");
        return [
    ...
    {
        return [
            "email" => $this->getConfigValue("rptech/quantumrma/recipient"),
            "name" => "Agent",
        ];
    ...
    {
        $copyTo = $this->getConfigValue("rptech/quantumrma/copy_to");
        if (!empty($copyTo)) {
