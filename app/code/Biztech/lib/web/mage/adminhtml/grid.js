    ...
    varienGridMassaction.prototype = {
        ...
        onConfirm: function (fieldName, item) {
            ...
            new Insertion.Bottom(this.formHiddens, this.fieldTemplate({
              ...
              /* start: Magemob Inventory Js changes for manage stock grid */
            if (this.currentItem.id == "updateInv") {
                checkeditems = this.checkedString.split(",");
                for (var i = 0; i < checkeditems.length; i++) {
                    if (checkeditems[i] != '') {
                        var result = {};

                        var totalQty = $('product[' + checkeditems[i] + '][total_qty]').value;
                        if (parseFloat(totalQty) % 1 != 0 && parseFloat(totalQty) % 1 != NaN) {
                            alert({
                                content: 'Please enter proper Total Qty.'
                            });
                            return false;
                        }

                        result.total_qty = $('product[' + checkeditems[i] + '][total_qty]').value;
                        result.comment = $('product[' + checkeditems[i] + '][comment]').value;
                        result.qty_level = $('product[' + checkeditems[i] + '][qty_level]').value;

                        this.finalresult.id = checkeditems[i];
                        this.finalresult.data = result;
                        this.finalresult1 = JSON.stringify(this.finalresult)
                        if (this.finalresult2 == "")
                            this.finalresult2 = this.finalresult1;
                        else
                            this.finalresult2 = this.finalresult2 + '$#$' + this.finalresult1;
                    }
                }
                var updateStockData = this.finalresult2.replace(/"/g, "'");
                if ($('warehouse_switcher') && $('warehouse_switcher') != undefined)
                    var current_warehouse = $('warehouse_switcher').value;
            }
            this.formHiddens.update('');
            new Insertion.Bottom(this.formHiddens, this.fieldTemplate({ name: fieldName, value: this.checkedString }));
            new Insertion.Bottom(this.formHiddens, this.fieldTemplate({ name: 'massaction_prepare_key', value: fieldName }));
            if (this.currentItem.id == "updateInv") {
                new Insertion.Bottom(this.formHiddens, this.fieldTemplate({ name: 'updatestockdata', value: updateStockData }));
                new Insertion.Bottom(this.formHiddens, this.fieldTemplate({ name: 'currentwarehouse', value: current_warehouse }));
            }
            /* End: Magemob Inventory Js changes for manage stock grid */
