...
class ExportSelectedProducts extends \Magento\Backend\App\Action
{
    ...
    /**
     * This function is used for export the selected products with warehouse qty details
     * @return Void
     */
    public function execute()
    {
        ...
        $collections->joinField('is_in_stock', $connection->getTableName('cataloginventory_stock_item'), 'is_in_stock', 'product_id=entity_id', '{{table}}.stock_id=1', 'left');

        /* pass filter to select products for export in csv */
        
        if ($this->request->getParam('inventorysystem')) {
            $collections->addAttributeToFilter(array(array('attribute' => 'entity_id', 'in' => $this->request->getParam('inventorysystem'))));
        }
