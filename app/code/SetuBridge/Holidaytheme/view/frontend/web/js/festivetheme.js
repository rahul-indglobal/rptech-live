...
define([
    'jquery',
    'SetuBridge_Holidaytheme/js/prototype'
    ],function(jQuery, prototype){
      sbSnowflakes = Class.create({
      initialize: function(userSettings) {
        var defaults = {
            ...
            elements: [
                {
                    html: '<span>&#x2744</span>',
                    color: '#ffffff'
                }
            ]
        },
        settings = jQuery.extend(defaults, userSettings),
        spawnLineStarts = {},
        spawnLineEnds = {},
        endLineStarts = {},
        endLineEnds = {},
        originOffset = {},
        jQueryelement = jQuery('<div class="flake" />').css({'position': 'absolute', 'top': '-50px'}).html('')
    ;
    ...
    return function () {
        ...
        var targetHeight =jQuery('body').height();
        var targetWidth = jQuery('body').width();
